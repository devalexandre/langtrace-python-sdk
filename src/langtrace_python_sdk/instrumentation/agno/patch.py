...
from langtrace_python_sdk.utils import set_span_attribute, handle_span_error
from langtrace_python_sdk.utils.llm import get_span_name, set_span_attributes, get_langtrace_attributes, set_event_completion, set_usage_attributes
...
def patch_google_genai(tracer: Tracer, version: str):
    def traced_method(wrapped, instance, args, kwargs):
        # Safely serialize the contents
        serialized_contents = _safe_serialize(kwargs.get("contents", []))
        prompt = [
            {
                "role": "user",
        ...
        # Create span attributes directly without using get_llm_request_attributes
        span_attributes = {
            "langtrace.sdk.name": LANGTRACE_SDK_NAME,
            "langtrace.service.name": "google_genai",
            "langtrace.service.type": "llm",
            "langtrace.service.version": version,
            "langtrace.version": v(LANGTRACE_SDK_NAME),
            "llm.prompts": json.dumps(prompt) if prompt else None,
            "llm.request.model": kwargs.get("model", "gemini"),
